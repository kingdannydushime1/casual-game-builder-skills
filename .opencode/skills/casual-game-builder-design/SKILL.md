...
1. **THE IDEA COMES FROM THE AGENT.** No user-supplied starting idea. The agent
   ...
   by itself. Never ask the user. **NEVER PRESENT A MENU OF IDEAS AND ASK THE
   USER TO CHOOSE** - the agent lists 5-8 candidate concepts (section 1.1),
   scores them itself, and commits to the best one. The user is only spoken to
   at delivery.
...
### 1.1 Generate a broad idea pool WITHIN the category - list, score, choose ALONE

The agent generates the pool, scores it, and picks the winner - **BY ITSELF,
NEVER consulting the user** (golden rule 11). The user is never asked "which
idea do you prefer?". Process:
...
- **SCORE EVERY IDEA on the 4 hit criteria (0-10 each), in a table, in
  GAMEDESIGN.md**:
  1. FUN IN 3 SECONDS: understood and rewarding instantly, no tutorial.
  2. SATISFYING: every action gives visible feedback (points pop, sound,
     shake, combo) - no dead moments.
  3. ADDICTIVE: "one more round" ending, short goal + progression (coins,
     unlocks), rising difficulty.
  4. HYPERCASUAL: ONE single gesture (tap, swipe, drag, aim), readable in a
     glance.
- **PICK THE WINNER BY SCORE, ALONE**: the idea with the best total wins.
  Never pick the safest, never pick a low-scoring idea, never ask the user.
  Record the scoring table + the choice + the reason in GAMEDESIGN.md.
...
- [ ] Idea pool: 5-8 concepts listed with the 4-hit-criteria scoring table and
      the winner chosen ALONE by the agent (never consulted)
...
- [ ] Idea generated by the agent (no user input) - pool listed, scored on the
      4 hit criteria (fun 3s, satisfying, addictive, hypercasual), winner
      chosen ALONE by the agent (never consulted) - with the anti-clone
